...
def addAnnotationFromGeometry(geom, name, pathClassName=null, color=null, imagePlane=null) {
    ...
    if (pathClassName != null) {
        pathClass = PathClass.fromString(pathClassName)
        //if (color != null)
...
if (!annTumor) {
    print "❌ Annotation 'tumor' not found"
    Dialogs.showErrorMessage("Error", "Annotation 'tumor' not found")
    return
}
...
// === Pomocná funkce ===
def fillAnnotationHoles(ann) {
    if (ann == null) return
    def geom = GeometryTools.roiToGeometry(ann.getROI())

    // buffer(0) opraví topologii → odstraní vnitřní díry
    def filled = geom.buffer(0)

    if (!filled.equalsExact(geom)) {
        def newROI = GeometryTools.geometryToROI(filled, ann.getROI().getImagePlane())
        ann.setROI(newROI)
        print "✅ Filled holes in annotation '${ann.getName()}'"
    } else {
        print "ℹ️ No holes to fill in '${ann.getName()}'"
    }
}


// === Použití pro tvé anotace
fillAnnotationHoles(annTumor)
fillAnnotationHoles(annSample)
fireHierarchyUpdate()
*/
...
/*
def fillHolesProperly(ann) {
    if (ann == null) return
    def geom = GeometryTools.roiToGeometry(ann.getROI())
    def gf = new GeometryFactory()

    if (geom.getGeometryType() == "Polygon") {
        // vytvoř polygon jen z vnějšího obrysu (bez vnitřních křivek)
        def outer = geom.getExteriorRing()
        def filled = gf.createPolygon(outer)
        ann.setROI(GeometryTools.geometryToROI(filled, ann.getROI().getImagePlane()))
        print "✅ Filled holes in '${ann.getName()}'"
    }
    else if (geom.getGeometryType() == "MultiPolygon") {
        def polys = (0..<geom.getNumGeometries()).collect { i ->
            def p = geom.getGeometryN(i)
            gf.createPolygon(p.getExteriorRing())
        }
        def filled = gf.createMultiPolygon(polys as Polygon[])
        ann.setROI(GeometryTools.geometryToROI(filled, ann.getROI().getImagePlane()))
        print "✅ Filled holes in '${ann.getName()}' (MultiPolygon)"
    }
} */
...
def fillHolesProperly(ann) {
    ...
    // 1️⃣ Opravíme topologii – odstraní samopřekryvy, degenerace apod.
    def cleaned = geom.buffer(0)

    // 2️⃣ Vytvoříme obrys bez děr (jen vnější ringy)
    def polygons = []
    if (cleaned.getGeometryType() == "Polygon") {
        polygons << gf.createPolygon(cleaned.getExteriorRing())
    } else if (cleaned.getGeometryType() == "MultiPolygon") {
        (0..<cleaned.getNumGeometries()).each { i ->
            def p = cleaned.getGeometryN(i)
            polygons << gf.createPolygon(p.getExteriorRing())
        }
    }

    // 3️⃣ Sloučíme vše do jedné oblasti (odstraní i mikrodíry)
    def merged = polygons.inject(polygons[0]) { acc, p -> acc.union(p) }

    // 4️⃣ Přepíšeme ROI
    def newROI = GeometryTools.geometryToROI(merged, ann.getROI().getImagePlane())
    ann.setROI(newROI)

    print "✅ Properly filled holes in '${ann.getName()}'"
}


fillHolesProperly(annTumor)
fillHolesProperly(annSample)
fireHierarchyUpdate()

print "✅ Filled holes in Tumor and Sample."
...
// make annTumor a bit larger

// === Turn polyline into JTS geometry ===
def roiTumor = annTumor.getROI()
def coords = roiTumor.getAllPoints().collect { p -> new Coordinate(p.getX(), p.getY()) }
...
def geomSelection = GeometryTools.roiToGeometry(annTumor.getROI())
